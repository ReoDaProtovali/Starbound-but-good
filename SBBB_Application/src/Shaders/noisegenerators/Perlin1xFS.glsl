...
float perlinNoise(vec3 position, uint seed) {
    position.y = abs(position.y + 100000.f); // bandaid solution;
...
void main() {
    ...
    float valR = cylinderNoise(uv, 1, 1, 0.5f, 2.f, uint(Seed));
    float valG = cylinderNoise(uv, 2, 1, 0.5f, 2.f, uint(Seed));
    float valB = cylinderNoise(uv, 5, 1, 0.5f, 2.f, uint(Seed));
    float valA = cylinderNoise(uv, 10, 1,0.5f, 2.f, uint(Seed));
